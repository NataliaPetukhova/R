...
---
```{r}
data <- read.csv2("C://Users//Natalia//Desktop//ITMO//R//CaseStudy#2models//AirQualityUCI.csv", header = TRUE, sep = ";")
head(data)              
```
...
```
```{r}
#normalise the data for the same scale:
dt <- scale(dt, center = TRUE, scale = TRUE)
```
...
k <- ols_step_all_possible(lmMod)
plot(k)
ols_step_best_subset(lmMod)
```

```{r}
#Check all variables for their p-values:

ols_step_forward_p(lmMod, penter = 0)
```
...
ggplot(train, aes(x = CO.GT., y = C6H6.GT.)) +
  ...
  theme_bw() +
  geom_label(aes(x = 5, y = 0), hjust = 0, 
...
model <- lm(C6H6.GT. ~.-PT08.S2.NMHC.-PT08.S4.NO2.-T -AH, data = train)
summary(model)
```
...
set.seed(42) 
sample <- sample.split(cdt, SplitRatio = 0.75)
trainm <- subset(cdt, sample == TRUE)
testm  <- subset(cdt, sample == FALSE)
...
```{r}
prognosis <- predict(model, newdata = testm)
head(prognosis)
```

```{r}
testm$C6H6.GT.pred <- prognosis
head(testm)
```

```{r}
ggplot(trainm, aes(x = CO.GT., y = C6H6.GT.)) +
  geom_point() +
  geom_smooth(method = "glm", color = "red" ) +
  geom_point(data = testm, aes(y = C6H6.GT.pred), color = "green") +
  theme_bw() +
  geom_label(aes(x = 5, y = 0), hjust = 0, 
...
```{r}
termplot(model, rug = TRUE, partial.resid = TRUE, se = TRUE, main = TRUE, smooth = panel.smooth, span.smth = 1/4)
```
